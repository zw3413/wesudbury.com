...
export default function OfferedRides({ translations, lang, driverEmail }: OfferedRidesProps) {
    ...
    const fetchRides = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/rides?page=${page}&limit=10&driverEmail=${encodeURIComponent(driverEmail)}`)
    ...
    return (<>
        {rides.length > 0 && (<>
            ...
            {(!hasMore && rides.length === 0) && (<p>{translations['noOfferedRides']}</p>)}
            {(!hasMore && rides.length > 0) && (<p>{translations['noMoreRides']}</p>)}
        </>
        )
        }</>)
