...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      const driverInfo = JSON.parse(Array.isArray(fields.driverInfo) ? fields.driverInfo[0] : fields.driverInfo || '{}')
      const vehicleInfo = JSON.parse(Array.isArray(fields.vehicleInfo) ? fields.vehicleInfo[0] : fields.vehicleInfo || '{}')
